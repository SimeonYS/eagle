...
class EaglePipeline:
    ...
    # Database setup
    conn = sqlite3.connect('eagle.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `eagle`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM eagle WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO eagle (date, title, link, content)"
